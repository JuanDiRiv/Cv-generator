...
function buildPrompt(
  ...
  if (field === "experience-description") {
    ...
    if (context?.title || context?.company) {
      base.push(
        `Role context (do not repeat in output): ${context.title ?? ""} at ${context.company ?? ""}.`,
      );
    }
  } else {
    base.push(
  ...
  try {
    ...
    if (!text || text.length < 10) {
      return Response.json(
        {
          error:
            "Escribe al menos una frase con lo que hiciste antes de pedir la reescritura.",
        },
        { status: 400 },
      );
    }
    if (text.length > 4000) {
      return Response.json(
        { error: "Texto demasiado largo (máx. 4000 caracteres)." },
        { status: 400 },
      );
    }
    ...
    if (!rewritten) {
      return Response.json(
        { error: "No se pudo generar la reescritura" },
        { status: 502 },
      );
    }
