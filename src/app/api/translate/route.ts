...
import type { CVDocument, ExperienceData, EducationData, AboutData } from '@/types/cv'

export async function POST(request: Request) {
  const { cv, targetLang }: { cv: CVDocument; targetLang: 'es' | 'en' } = await request.json()
  ...
  if (aboutData?.summary && aboutSection) {
    ;(translated.sections.find(s => s.type === 'about')!.data as AboutData).summary = results[i++]
  }
  expData?.entries.forEach((_, idx) => {
    if (expData.entries[idx].title) (translated.sections.find(s => s.type === 'experience')!.data as ExperienceData).entries[idx].title = results[i++]
    if (expData.entries[idx].description) (translated.sections.find(s => s.type === 'experience')!.data as ExperienceData).entries[idx].description = results[i++]
  })
  eduData?.entries.forEach((_, idx) => {
    if (eduData.entries[idx].degree) (translated.sections.find(s => s.type === 'education')!.data as EducationData).entries[idx].degree = results[i++]
    if (eduData.entries[idx].description) (translated.sections.find(s => s.type === 'education')!.data as EducationData).entries[idx].description = results[i++]
  })
